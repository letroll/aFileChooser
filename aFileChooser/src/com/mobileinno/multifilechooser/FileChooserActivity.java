...
public class FileChooserActivity extends FragmentActivity implements
    ...
    /**
     * "Replace" the existing Fragment with a new one using given path. We're really adding a Fragment to the back stack.
     *
     ...
     */
    private void finishWithResult(ArrayList<File> files) {
	if (files != null) {
	    for (File f : files) {
		Log.d("files", f.getAbsolutePath());
	    }
     ...
     */
    protected void onDecisionMade(ArrayList<File> files) {
